...
- **版本收尾清单**（每个版本发布前必做，见 `engineering.md` §5 发布流程 + §4「发布前核对流程」）
...
**新发现任何缺陷 → 先固化回归再继续**：凡是本次修的问题，立即在 `packages/ui/qa-regression/<组件>.spec.ts` 加一条断言（跨组件/全局场景进 `misc.spec.ts`，新组件新建对应文件），防止复发。只修不加回归 = 质量问题没闭环。
