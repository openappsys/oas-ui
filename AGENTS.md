...
- **主 agent / @coder 分工**：主 agent（当前对话）负责规划、看图识图（截图审查 demo）、复核与提交；编码实现派 `@coder` 子 agent（`.opencode/agents/coder.md`，deepseek 模型）。coder 干不了/干不好的（复杂交互、设计级重构、需视觉判断的）由主 agent 亲自实现。无关联任务并发派多个 @coder。
- **主 agent 复核义务**：每个 coder 任务交付后，主 agent 必须跑一遍「用户视角验证门禁」再提交——**coder 只报"机制绿"，视觉/交互/感知的最终判定在主 agent**，不能直接信测试结果就提交。coder 报告说"待复核"的点必须逐条落实。
- **需求收件箱**：临时需求/bug 记录到 `.opencode/demands.md`（在 `.git/info/exclude`，不进 git 仓库），主 agent 每批之间检查并安排处理。
