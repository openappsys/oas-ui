  ...
  for (let i = 0; i < section.length; i++) {
    ...
    if (cls.type === 'attr') {
      ...
      for (const row of rows) {
        ...
        if (!name) {
          skipped.rows.push({ file, header: headerCells.join(' | '), rowName: row.join(' | '), reason: '属性名缺失' });
          continue;
        }
        if (/^(无|无属性|none|no attributes)$/i.test(name)) {
          skipped.rows.push({ file, header: headerCells.join(' | '), rowName: row.join(' | '), reason: '无属性占位行，跳过' });
          continue;
        }
        ...
        if (CJK_RE.test(name) || /\s/.test(name)) {
          skipped.rows.push({ file, header: headerCells.join(' | '), rowName: row.join(' | '), reason: '非属性行（属性名非法/缺名）' });
          continue;
        }
    ...
    if (cls.type === 'event') {
      ...
      for (const row of rows) {
        ...
        if (!name) {
          skipped.rows.push({ file, header: headerCells.join(' | '), rowName: row.join(' | '), reason: '事件名缺失' });
          continue;
        }
    ...
    if (cls.type === 'slot') {
      ...
      for (const row of rows) {
        ...
        if (!raw.trim()) {
          skipped.rows.push({ file, header: headerCells.join(' | '), rowName: row.join(' | '), reason: '插槽名缺失' });
          continue;
        }
...
function printReport(zh, en) {
  ...
  for (const [lang, res] of [['中文', zh], ['英文', en]]) {
    const s = res.stats;
