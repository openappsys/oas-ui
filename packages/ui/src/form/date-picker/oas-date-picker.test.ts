...
describe('OASDatePicker 移动端底部抽屉（bottom-sheet 接入）', () => {
  beforeEach(() => {
    document.body.innerHTML = ''
  })
  ...
  function sheet(el: OASDatePicker): HTMLElement {
  ...
  it('模板恒包 oas-bottom-sheet（SSR/客户端结构一致），PC 默认 passive 透传', () => {
    ...
    expect(sheet(el).contains(el.shadowRoot!.querySelector('[part="dropdown"]')!)).toBe(true)
  ...
  it('触屏进入移动形态：去 passive + data-mobile-sheet，展开走 sheet 且跳过浮层定位，oas-close 同步收起', () => {
    ...
    const callsBefore = computePositionMock.mock.calls.length
    open(el)
    expect(expanded(el)).toBe('true')
    expect(sheet(el).hasAttribute('open')).toBe(true)
    // 移动形态跳过 fixed 锚定（原定位由 bottom-sheet 容器承载）
    expect(computePositionMock.mock.calls.length).toBe(callsBefore)
