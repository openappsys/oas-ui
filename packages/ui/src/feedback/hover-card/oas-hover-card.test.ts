...
describe('OASHoverCard', () => {
  ...
  describe('title 吸收（消除宿主原生 tooltip）', () => {
    it('挂载后宿主不再残留 title 属性，标题照常渲染进卡片标题区', () => {
      const el = mount({ open: '', title: '卡片标题' })
      expect(el.hasAttribute('title'), '宿主原生 title 应被吸收移除').toBe(false)
      expect(card(el).querySelector('[part="title"]')!.textContent).toBe('卡片标题')
    })
    ...
    it('运行时改 title 属性：新值吸收渲染，宿主仍无残留', () => {
      ...
      expect(card(el).querySelector('[part="title"]')!.textContent).toBe('新标题')
      expect(el.hasAttribute('title')).toBe(false)
    })
    ...
    it('title="" 清空标题（属性在场=宿主意图）', () => {
      ...
      expect(card(el).querySelector('[part="title"]')!.textContent).toBe('')
      expect(el.hasAttribute('title')).toBe(false)
    })

    it('水合恢复：快照标题区有文本时恢复缓存，水合后标题不丢失', () => {
      const ref = new OASHoverCard()
      ref.setAttribute('title', '水合卡片')
