...
import { isRtl, directionOf } from './direction.js'
...
describe('isRtl / directionOf 书写方向判定', () => {
  it('最近 [dir=rtl] → RTL', () => {
    expect(isRtl(host('rtl'))).toBe(true)
    expect(directionOf(host('rtl'))).toBe('rtl')
  })
  ...
  it('就近声明优先：外层 rtl、内层 ltr → 内层胜', () => {
    ...
    expect(directionOf(inner)).toBe('ltr')
  })

  it('无显式 dir 回退计算样式（默认 LTR）', () => {
    expect(isRtl(host())).toBe(false)
    expect(directionOf(host())).toBe('ltr')
  })
