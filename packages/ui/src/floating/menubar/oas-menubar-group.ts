...
const GROUP_STYLE = `
:host {
  /* 数据载体：自身不渲染任何可视内容，由 <oas-menubar> 解析 label/value 属性与子元素为内部 items 模型后统一渲染 */
  display: none;
}
`
 ...
 * 子元素 <oas-menubar-item>/<oas-menubar-group>/<oas-menubar-divider> 递归为平铺 children
 * （group 的 children 平铺同层，对齐 items 字段既有语义）。
 * 宿主 <oas-menubar> 在 items 属性未显式设置时解析子元素并收敛到同一渲染路径。
 */
export class OASMenubarGroup extends OASElement {
