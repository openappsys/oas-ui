...
/**
 * `<oas-config-provider locale size theme config direction z-index>` —— 全局配置注入入口。
 *
 ...
 *   组件经 core `readConfigValue(el, tag, key)` 就近读取；非法 JSON 忽略 + dev 告警（同值去重）
...
export class OASConfigProvider extends OASElement {
  static override get observedAttributes(): string[] {
    return ['locale', 'size', 'theme', 'config', 'direction', 'z-index']
  }
