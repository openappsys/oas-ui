 ...
 * OASTableBase 构造时遍历注册表快照注入已注册能力，并经 `onTableCapabilityRegistered` 订阅
...
/** 注册一个 table 能力包（重复注册同名能力被幂等忽略；新注册时通知全部晚加入监听） */
 ...
 */
export function onTableCapabilityRegistered(cb: () => void): () => void {
