// 复核回归：backdrop——能力做透缺陷固化（内容插槽点击判定 / persistent shake 反馈 /
...
test('backdrop 内容插槽：点击内容区不关闭、点击遮罩本体关闭（P1+P5 点击判定）', async ({
  ...
  await page.waitForSelector('#backdrop-loading')
...
test('backdrop persistent：点击遮罩不关闭、内容 shake 反馈、内容区按钮关闭', async ({ page }) => {
  ...
  await page.waitForSelector('#backdrop-persistent')
  await page.waitForFunction(
    () =>
      document
...
test('backdrop 颜色/浓度/模糊属性注入 scrim（demo 属性存活）', async ({ page }) => {
  ...
  await page.waitForSelector('#backdrop-blur-strong')
  await page.waitForFunction(
    () => {
      const scrim = document
