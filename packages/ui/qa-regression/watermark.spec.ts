...
test('canvas 引擎：真实浏览器文字水印背景为 data:image/png（非 SVG 回退）', async ({
  page,
}) => {
...
test('grayscale：灰阶 tile 与原图 tile 的 dataURL 不同（canvas filter 钩子生效）', async ({
  page,
}) => {
  await page.goto('/components/watermark.html', { waitUntil: 'domcontentloaded' })
  ...
  await page.waitForFunction(() => {
    const layer = document
      .querySelector('#wm-img-gray')
      ?.shadowRoot?.querySelector('[part="watermark"]')
...
test('fullscreen：fixed 定位铺满视口、pointer-events none、默认最高层级，z-index 属性可覆盖', async ({
  page,
}) => {
  await page.goto('/components/watermark.html', { waitUntil: 'domcontentloaded' })
  ...
  const z = await page.evaluate(
    () => (document.querySelector('#wm-fullscreen-demo') as HTMLElement).style.zIndex,
  )
  expect(z).toBe('999')
...
test('movable：canvas 引擎下拖拽 offset 写回、背景位置跟随（图层机制不因引擎迁移失效）', async ({
  page,
}) => {
  await page.goto('/components/watermark.html', { waitUntil: 'domcontentloaded' })
...
test('watermark demo 属性存活（Vue 不剥离）：grayscale / fullscreen 宿主属性均在', async ({
  page,
}) => {
  await page.goto('/components/watermark.html', { waitUntil: 'domcontentloaded' })
