...
test('堆叠布局：三条同向打开纵向排列不重叠（最新贴底边）', async ({ page }) => {
  ...
  await page.waitForFunction(() => document.querySelectorAll('oas-snackbar.oas-open').length === 3, null, {
    timeout: 5000,
  })
...
test('堆叠超限挤掉最老：第 4 条打开时最老一条收到 reason=evict 的 oas-close', async ({ page }) => {
  ...
  await page.waitForFunction(() => document.querySelectorAll('oas-snackbar.oas-open').length === 3)
  await page.evaluate(() => (window as any).sbShow({ message: '第四条', duration: '0' }))
  await page.waitForFunction(() => (document.getElementById('sb-log')?.textContent ?? '').includes('evict'), null, {
    timeout: 5000,
  })
  // 挤掉后仍保持 3 条在屏
...
test('closable 关闭按钮：点击后 open 移除且 reason=close', async ({ page }) => {
  await page.evaluate(() => (window as any).sbShow({ message: '常驻可关', duration: '0', closable: '' }))
  ...
  await page.waitForFunction(() => (document.getElementById('sb-log')?.textContent ?? '').includes('close'), null, {
    timeout: 5000,
  })
...
test('同内容合并：group 同文案连发两次合并为一条且计数 ×2', async ({ page }) => {
  ...
  await groupBlock.locator('oas-button').first().click()
  await page.waitForFunction(() => document.querySelectorAll('oas-snackbar.oas-open').length === 1, null, {
    timeout: 5000,
  })
...
test('Escape 关闭：无焦点归属时关最老一条（reason=escape）', async ({ page }) => {
  ...
  await page.waitForFunction(() => document.querySelectorAll('oas-snackbar.oas-open').length === 2)
  await page.keyboard.press('Escape')
  await page.waitForFunction(() => (document.getElementById('sb-log')?.textContent ?? '').includes('escape'), null, {
    timeout: 5000,
  })
  // 关的是最老一条，最新仍在屏
...
test('计时进度条：progress 开启时可见且随时间推进（scaleX 动画）', async ({ page }) => {
  await page.evaluate(() => (window as any).sbShow({ message: '带进度', duration: '6000', progress: '' }))
  await page.waitForFunction(() => document.querySelectorAll('oas-snackbar.oas-open').length === 1)
