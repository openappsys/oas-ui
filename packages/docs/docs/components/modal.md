...
**受控 / 非受控契约**：组件自身是非受控 + 事件通知模型——用户关闭路径（遮罩 / ✕ / Esc / 取消）由组件自行移除 `visible` 并派发 `oas-close`（`detail.source` / `detail.action` 标明来源与动作），宿主上的 `visible` attribute 始终与真实状态同步、不会残留。声明式绑定场景（`visible` 由宿主 state 驱动）应在 `oas-close` 中把 state 置为关闭，避免关闭后 state 又把 `visible` 写回；若监听 `oas-before-close` 并 `preventDefault()`，则组件保持打开、`visible` 不动。
...
> **prompt 能力（主路径已默认内含）**：主路径 `@oas-ui/ui/feedback/modal` 已内置 prompt 能力（import 即注册），无需显式引用。若想保留不含 prompt machinery 的纯核瘦身（确认/提示类消费者零开销），可改从纯核入口 `@oas-ui/ui/feedback/modal/core` 引入——它不含该能力，此时调用 `modal.prompt` 返回 `null` 并在 dev 控制台告警一次，需显式引用：
>
> ```ts
> import '@oas-ui/ui/feedback/modal/prompt'
> ```
>
> 或换回主路径。全量入口 `@oas-ui/ui` 与 CDN 反馈族包同样已内含。
