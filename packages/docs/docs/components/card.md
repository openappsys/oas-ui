...
单卡缺省不挂 `role`：可点/可选卡内常嵌操作按钮或链接，挂交互角色会把卡内控件裹进交互元素（屏幕阅读器丢失内部控件可读性），组件仅保留 `tabindex` 保证键盘可达；**多卡组需要单选/多选语义时**，由宿主在卡片上显式挂 `role="radio"`（或 `role="checkbox"`）并自行管理互斥选中——组件不覆盖宿主显式角色，只叠加 `aria-checked` 同步。
