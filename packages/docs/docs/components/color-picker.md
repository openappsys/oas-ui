...
> **designer 能力（主路径已默认内含）**：主路径 `@oas-ui/ui/form/color-picker` 已内置 2D 色域 / hue 竖条 / 渐变（`mode="gradient"`）设计器（import 即注册），无需显式引用。仅使用默认能力（swatch 预设 + hex/RGB 输入 + alpha）的消费者可改从纯核入口 `@oas-ui/ui/form/color-picker/core` 引入瘦身——它不含该能力，此时 `mode="gradient"` 配置静默失效并 dev 告警，需显式 `import '@oas-ui/ui/form/color-picker/designer'`（import 即注册）或换回主路径。全量入口 `@oas-ui/ui` 与 CDN 表单族包同样已内含。
