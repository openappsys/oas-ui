...
> **Designer capability ships with the main entry**: the main entry `@oas-ui/ui/form/color-picker` already bundles the 2D color field / vertical hue rail / gradient editor (`mode="gradient"`, including the gradient demos below and the inline gradient form; import registers it) — no explicit import needed. Consumers that only need the default behavior (swatch presets + hex/RGB input + alpha) can slim down with the pure-core entry `@oas-ui/ui/form/color-picker/core` — it omits the capability, so `mode="gradient"` silently degrades with a dev hint; explicitly `import '@oas-ui/ui/form/color-picker/designer'` (import registers it) or switch back to the main entry. The full `@oas-ui/ui` entry and the CDN form-family bundles include it too.
