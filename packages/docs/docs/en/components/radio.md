...
<style>
/* Unchecked: ::part(box) customizes the ring (dashed + secondary color); checked: primary solid
   ring + the slot's primary filled star. Only rock-solid primitives are used (::part() +
   currentColor — no mask, no var() inside SVG attributes) so every engine renders it the same. */
.radio-star::part(box) {
...
<DemoBlock title="required (native validation chain)">
  <oas-radio required name="plan" value="a">Plan A</oas-radio>
</DemoBlock>
