...
| **v2.1.6** | 导航与浮层族 11 组件能力补齐（能力并集，能力横向补齐 A/B/C 全做）：tooltip（placement 12 向/trigger 多选/延迟分离/富内容/Esc+describedby/interactive/动画/skip-delay/portal/color 变体/C 档独家全做）+ popover（hover 触发/width 同宽/初始焦点/关闭按钮/modal 化/碰撞细调）+ hover-card（浮层可悬停立身修复/富内容/箭头/role 修正）+ breadcrumb（icon/图标分隔符/真实链接/折叠可配/项下拉/键盘导航/JSON-LD SEO）+ anchor（滚动容器/落点偏移/嵌套/横向/滚动联动/墨水条/affix/oas-anchor-target）+ back-top（visibility-height 自动显隐/target/插槽/进度环/expand/reverse）+ tour（placement 12 向/滚动重定位/step 级覆盖/hints 信标/不再显示记忆/多页引导）+ command（图标/kbd 标注/loading/外部过滤/hotkey 可配/open-change/嵌套页面/最近使用/虚拟滚动/多选；豁免：内置模糊搜索并入打包勾选特性）+ menubar（checkbox/typeahead/open 受控/click 首开/汉堡收纳/竖排）+ navigation-menu（大面板网格/延迟开合/viewport 动画/indicator/backdrop；disclosure navigation 语义）+ toolbar（toggle/separator/input 新部件/orientation/溢出收纳/is-attached）+ 浮层定位引擎 12 向升级 + 实测修复（toolbar SSR 误判/popover aria-hidden 滞后/back-top 水合竞态） | ✅ 已完成 |
