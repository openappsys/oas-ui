...
- **渲染基准（本地/发布前）**：`pnpm perf:bench`。happy-dom 环境测代表性组件首渲染与增量更新耗时（100 次迭代取均值/中位数），量级断言防退化。渲染耗时波动大，**不进 CI**；发布前本地跑一遍核对。同样**默认只测不写**，更新基线加 `--update-baseline`。
