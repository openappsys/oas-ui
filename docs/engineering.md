...
- AI 改代码前必须读对应组件测试与 ui-spec，不得绕过测试直接"补实现"
- AI 并行开发（多个子任务）必须互不重叠：每个组件独立目录，公共设施先定接口再实现
- AI 产出必须自查并运行对应测试，不得声称"应该能过"
- **主 agent / @coder 分工**：主 agent（当前对话）负责规划、看图识图（截图审查 demo）、复核与提交；编码实现派给 `@coder` 子 agent（`.opencode/agents/coder.md`，deepseek 模型）。coder 干不了/干不好的（复杂交互、设计级重构）由主 agent 亲自实现。无关联任务可并发派多个 @coder。
- **需求收件箱**：临时需求/bug 记录到 `.opencode/demands.md`（在 git exclude，不进仓库），主 agent 每批之间检查并安排处理。
